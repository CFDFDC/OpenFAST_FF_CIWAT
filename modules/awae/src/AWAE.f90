...
!#endif 
   ...
   do i = 0 , p%NumGrid_low - 1
               ...
               do np = 0, maxPln
                  ...
                  if ( ( ( x_start_plane >= 0.0_ReKi ) .and. ( x_end_plane < 0.0_ReKi ) ) .or. &
                     ...
                     if ( r_tmp_plane <= p%r(p%numRadii-1) ) then
                        ...
                        if ( tmp_k_mt_interp > tmp_k_mt) THEN
        ...
        if (n_wake > 0) then
           ...
           if (abs( (m%Vamb_low(1,nx_low,ny_low,nz_low) - m%Vdist_low(1,nx_low,ny_low,nz_low)) / m%Vamb_low(1,nx_low,ny_low,nz_low) ) > 0.01_ReKi) then
           ! (CIWAT) Band-pass filtering of upstream disturbed velocity   
           !p%filtParam_WkAdT_lower = exp(-2.0_ReKi*pi*p%dt_low*(0.1_ReKi*8.0_ReKi/126.0_ReKi))       
           !p%filtParam_WkAdT_upper = exp(-2.0_ReKi*pi*p%dt_low*(1.0_ReKi*8.0_ReKi/126.0_ReKi))      
           !m%TurbAmb_low_bandPass(:,nx_low,ny_low,nz_low)= m%TurbAmb_low_bandPass(:,nx_low,ny_low,nz_low) * p%filtParam_WkAdT_upper + m%TurbAmb_low_highPass(:,nx_low,ny_low,nz_low)* (1.0_ReKi - p%filtParam_WkAdT_upper)
           !m%TurbAmb_low_highPass(:,nx_low,ny_low,nz_low)= m%Vamb_low(:,nx_low,ny_low,nz_low) - m%Vamb_low_lowPass(:,nx_low,ny_low,nz_low)
           !m%Vamb_low_lowPass(:,nx_low,ny_low,nz_low) = m%Vamb_low_lowPass(:,nx_low,ny_low,nz_low) * p%filtParam_WkAdT_lower + m%Vamb_low(:,nx_low,ny_low,nz_low) * (1.0_ReKi - p%filtParam_WkAdT_lower)
           !m%Vdist_low(:,nx_low,ny_low,nz_low) = m%Vdist_low(:,nx_low,ny_low,nz_low) + tmp_k_mt * m%TurbAmb_low_bandPass(:,nx_low,ny_low,nz_low)   
           m%TurbDist_low_bandPass(:,nx_low,ny_low,nz_low) = m%TurbDist_low_bandPass(:,nx_low,ny_low,nz_low) * exp(-2.0_ReKi*pi*p%dt_low*(1.0_ReKi*8.0_ReKi/126.0_ReKi)) & 
               + m%TurbDist_low_highPass(:,nx_low,ny_low,nz_low)* (1.0_ReKi - exp(-2.0_ReKi*pi*p%dt_low*(1.0_ReKi*8.0_ReKi/126.0_ReKi)))
           m%TurbDist_low_highPass(:,nx_low,ny_low,nz_low) = m%Vdist_low(:,nx_low,ny_low,nz_low) - m%Vdist_low_lowPass(:,nx_low,ny_low,nz_low)
           m%Vdist_low_lowPass(:,nx_low,ny_low,nz_low) = m%Vdist_low_lowPass(:,nx_low,ny_low,nz_low) * exp(-2.0_ReKi*pi*p%dt_low*(0.1_ReKi*8.0_ReKi/126.0_ReKi)) &
               + m%Vdist_low(:,nx_low,ny_low,nz_low) * (1.0_ReKi - exp(-2.0_ReKi*pi*p%dt_low*(0.1_ReKi*8.0_ReKi/126.0_ReKi)))
           
           m%Vdist_low(:,nx_low,ny_low,nz_low) = m%Vdist_low(:,nx_low,ny_low,nz_low) + (tmp_k_mt +0.5_ReKi)* m%TurbDist_low_bandPass(:,nx_low,ny_low,nz_low)                 
           end if
        end if  ! (n_wake > 0)
   end do
...
!..................................................................................................................................
   ...
   allocate ( u%k_mt      (0:p%NumRadii-1, 0:p%NumPlanes-1, 1:p%NumTurbines), STAT=ErrStat2 ) ! Wake added turbulence
   ...
   ! (CIWAT)
   allocate ( m%Vdist_low_lowPass   ( 3, 0:p%nX_low-1 , 0:p%nY_low-1 , 0:p%nZ_low-1 )                  , STAT=errStat2 )
      if (errStat2 /= 0) call SetErrStat ( ErrID_Fatal, 'Could not allocate memory for m%Vamb_low_lowPass.', errStat, errMsg, RoutineName )
   allocate ( m%TurbDist_low_highPass   ( 3, 0:p%nX_low-1 , 0:p%nY_low-1 , 0:p%nZ_low-1 )                  , STAT=errStat2 )
      if (errStat2 /= 0) call SetErrStat ( ErrID_Fatal, 'Could not allocate memory for m%TurbDist_low_highPass.', errStat, errMsg, RoutineName )
   allocate ( m%TurbDist_low_bandPass   ( 3, 0:p%nX_low-1 , 0:p%nY_low-1 , 0:p%nZ_low-1 )                  , STAT=errStat2 )
      if (errStat2 /= 0) call SetErrStat ( ErrID_Fatal, 'Could not allocate memory for m%TurbDist_low_bandPass.', errStat, errMsg, RoutineName )  
