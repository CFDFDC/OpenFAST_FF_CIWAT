...
! =========  AWAE_MiscVarType  =======
  TYPE, PUBLIC :: AWAE_MiscVarType
    ...
    REAL(SiKi) , DIMENSION(:,:,:,:), ALLOCATABLE  :: Vdist_low_lowPass      !< UVW components of low-pass velocity across the low-resolution domain throughout the farm [m/s]
    REAL(SiKi) , DIMENSION(:,:,:,:), ALLOCATABLE  :: TurbDist_low_highPass      !< UVW components of high-pass velocity fluctuations across the low-resolution domain throughout the farm [m/s]
    REAL(SiKi) , DIMENSION(:,:,:,:), ALLOCATABLE  :: TurbDist_low_bandPass      !< UVW components of band-pass velocity fluctuations across the low-resolution domain throughout the farm [m/s]
...
ENDIF
IF (ALLOCATED(SrcMiscData%Vdist_low_lowPass)) THEN
  i1_l = LBOUND(SrcMiscData%Vdist_low_lowPass,1)
  i1_u = UBOUND(SrcMiscData%Vdist_low_lowPass,1)
  i2_l = LBOUND(SrcMiscData%Vdist_low_lowPass,2)
  i2_u = UBOUND(SrcMiscData%Vdist_low_lowPass,2)
  i3_l = LBOUND(SrcMiscData%Vdist_low_lowPass,3)
  i3_u = UBOUND(SrcMiscData%Vdist_low_lowPass,3)
  i4_l = LBOUND(SrcMiscData%Vdist_low_lowPass,4)
  i4_u = UBOUND(SrcMiscData%Vdist_low_lowPass,4)
  IF (.NOT. ALLOCATED(DstMiscData%Vdist_low_lowPass)) THEN 
    ALLOCATE(DstMiscData%Vdist_low_lowPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%Vdist_low_lowPass.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%Vdist_low_lowPass = SrcMiscData%Vdist_low_lowPass
ENDIF
IF (ALLOCATED(SrcMiscData%TurbDist_low_highPass)) THEN
  i1_l = LBOUND(SrcMiscData%TurbDist_low_highPass,1)
  i1_u = UBOUND(SrcMiscData%TurbDist_low_highPass,1)
  i2_l = LBOUND(SrcMiscData%TurbDist_low_highPass,2)
  i2_u = UBOUND(SrcMiscData%TurbDist_low_highPass,2)
  i3_l = LBOUND(SrcMiscData%TurbDist_low_highPass,3)
  i3_u = UBOUND(SrcMiscData%TurbDist_low_highPass,3)
  i4_l = LBOUND(SrcMiscData%TurbDist_low_highPass,4)
  i4_u = UBOUND(SrcMiscData%TurbDist_low_highPass,4)
  IF (.NOT. ALLOCATED(DstMiscData%TurbDist_low_highPass)) THEN 
    ALLOCATE(DstMiscData%TurbDist_low_highPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%TurbDist_low_highPass.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%TurbDist_low_highPass = SrcMiscData%TurbDist_low_highPass
ENDIF
IF (ALLOCATED(SrcMiscData%TurbDist_low_bandPass)) THEN
  i1_l = LBOUND(SrcMiscData%TurbDist_low_bandPass,1)
  i1_u = UBOUND(SrcMiscData%TurbDist_low_bandPass,1)
  i2_l = LBOUND(SrcMiscData%TurbDist_low_bandPass,2)
  i2_u = UBOUND(SrcMiscData%TurbDist_low_bandPass,2)
  i3_l = LBOUND(SrcMiscData%TurbDist_low_bandPass,3)
  i3_u = UBOUND(SrcMiscData%TurbDist_low_bandPass,3)
  i4_l = LBOUND(SrcMiscData%TurbDist_low_bandPass,4)
  i4_u = UBOUND(SrcMiscData%TurbDist_low_bandPass,4)
  IF (.NOT. ALLOCATED(DstMiscData%TurbDist_low_bandPass)) THEN 
    ALLOCATE(DstMiscData%TurbDist_low_bandPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%TurbDist_low_bandPass.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%TurbDist_low_bandPass = SrcMiscData%TurbDist_low_bandPass
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%Vdist_low_lowPass)) THEN
  DEALLOCATE(MiscData%Vdist_low_lowPass)
ENDIF
IF (ALLOCATED(MiscData%TurbDist_low_highPass)) THEN
  DEALLOCATE(MiscData%TurbDist_low_highPass)
ENDIF
IF (ALLOCATED(MiscData%TurbDist_low_bandPass)) THEN
  DEALLOCATE(MiscData%TurbDist_low_bandPass)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! Vdist_low_lowPass allocated yes/no
  IF ( ALLOCATED(InData%Vdist_low_lowPass) ) THEN
    Int_BufSz   = Int_BufSz   + 2*4  ! Vdist_low_lowPass upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%Vdist_low_lowPass)  ! Vdist_low_lowPass
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! TurbDist_low_highPass allocated yes/no
  IF ( ALLOCATED(InData%TurbDist_low_highPass) ) THEN
    Int_BufSz   = Int_BufSz   + 2*4  ! TurbDist_low_highPass upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TurbDist_low_highPass)  ! TurbDist_low_highPass
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! TurbDist_low_bandPass allocated yes/no
  IF ( ALLOCATED(InData%TurbDist_low_bandPass) ) THEN
    Int_BufSz   = Int_BufSz   + 2*4  ! TurbDist_low_bandPass upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TurbDist_low_bandPass)  ! TurbDist_low_bandPass
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%Vdist_low_lowPass) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Vdist_low_lowPass,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Vdist_low_lowPass,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Vdist_low_lowPass,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Vdist_low_lowPass,2)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Vdist_low_lowPass,3)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Vdist_low_lowPass,3)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Vdist_low_lowPass,4)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Vdist_low_lowPass,4)
    Int_Xferred = Int_Xferred + 2

      DO i4 = LBOUND(InData%Vdist_low_lowPass,4), UBOUND(InData%Vdist_low_lowPass,4)
        DO i3 = LBOUND(InData%Vdist_low_lowPass,3), UBOUND(InData%Vdist_low_lowPass,3)
          DO i2 = LBOUND(InData%Vdist_low_lowPass,2), UBOUND(InData%Vdist_low_lowPass,2)
            DO i1 = LBOUND(InData%Vdist_low_lowPass,1), UBOUND(InData%Vdist_low_lowPass,1)
              ReKiBuf(Re_Xferred) = InData%Vdist_low_lowPass(i1,i2,i3,i4)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%TurbDist_low_highPass) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_highPass,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_highPass,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_highPass,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_highPass,2)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_highPass,3)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_highPass,3)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_highPass,4)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_highPass,4)
    Int_Xferred = Int_Xferred + 2

      DO i4 = LBOUND(InData%TurbDist_low_highPass,4), UBOUND(InData%TurbDist_low_highPass,4)
        DO i3 = LBOUND(InData%TurbDist_low_highPass,3), UBOUND(InData%TurbDist_low_highPass,3)
          DO i2 = LBOUND(InData%TurbDist_low_highPass,2), UBOUND(InData%TurbDist_low_highPass,2)
            DO i1 = LBOUND(InData%TurbDist_low_highPass,1), UBOUND(InData%TurbDist_low_highPass,1)
              ReKiBuf(Re_Xferred) = InData%TurbDist_low_highPass(i1,i2,i3,i4)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%TurbDist_low_bandPass) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_bandPass,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_bandPass,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_bandPass,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_bandPass,2)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_bandPass,3)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_bandPass,3)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TurbDist_low_bandPass,4)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TurbDist_low_bandPass,4)
    Int_Xferred = Int_Xferred + 2

      DO i4 = LBOUND(InData%TurbDist_low_bandPass,4), UBOUND(InData%TurbDist_low_bandPass,4)
        DO i3 = LBOUND(InData%TurbDist_low_bandPass,3), UBOUND(InData%TurbDist_low_bandPass,3)
          DO i2 = LBOUND(InData%TurbDist_low_bandPass,2), UBOUND(InData%TurbDist_low_bandPass,2)
            DO i1 = LBOUND(InData%TurbDist_low_bandPass,1), UBOUND(InData%TurbDist_low_bandPass,1)
              ReKiBuf(Re_Xferred) = InData%TurbDist_low_bandPass(i1,i2,i3,i4)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
      ...
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! Vdist_low_lowPass not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i4_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%Vdist_low_lowPass)) DEALLOCATE(OutData%Vdist_low_lowPass)
    ALLOCATE(OutData%Vdist_low_lowPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%Vdist_low_lowPass.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i4 = LBOUND(OutData%Vdist_low_lowPass,4), UBOUND(OutData%Vdist_low_lowPass,4)
        DO i3 = LBOUND(OutData%Vdist_low_lowPass,3), UBOUND(OutData%Vdist_low_lowPass,3)
          DO i2 = LBOUND(OutData%Vdist_low_lowPass,2), UBOUND(OutData%Vdist_low_lowPass,2)
            DO i1 = LBOUND(OutData%Vdist_low_lowPass,1), UBOUND(OutData%Vdist_low_lowPass,1)
              OutData%Vdist_low_lowPass(i1,i2,i3,i4) = REAL(ReKiBuf(Re_Xferred), SiKi)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TurbDist_low_highPass not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i4_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TurbDist_low_highPass)) DEALLOCATE(OutData%TurbDist_low_highPass)
    ALLOCATE(OutData%TurbDist_low_highPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TurbDist_low_highPass.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i4 = LBOUND(OutData%TurbDist_low_highPass,4), UBOUND(OutData%TurbDist_low_highPass,4)
        DO i3 = LBOUND(OutData%TurbDist_low_highPass,3), UBOUND(OutData%TurbDist_low_highPass,3)
          DO i2 = LBOUND(OutData%TurbDist_low_highPass,2), UBOUND(OutData%TurbDist_low_highPass,2)
            DO i1 = LBOUND(OutData%TurbDist_low_highPass,1), UBOUND(OutData%TurbDist_low_highPass,1)
              OutData%TurbDist_low_highPass(i1,i2,i3,i4) = REAL(ReKiBuf(Re_Xferred), SiKi)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TurbDist_low_bandPass not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i4_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TurbDist_low_bandPass)) DEALLOCATE(OutData%TurbDist_low_bandPass)
    ALLOCATE(OutData%TurbDist_low_bandPass(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u,i4_l:i4_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TurbDist_low_bandPass.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i4 = LBOUND(OutData%TurbDist_low_bandPass,4), UBOUND(OutData%TurbDist_low_bandPass,4)
        DO i3 = LBOUND(OutData%TurbDist_low_bandPass,3), UBOUND(OutData%TurbDist_low_bandPass,3)
          DO i2 = LBOUND(OutData%TurbDist_low_bandPass,2), UBOUND(OutData%TurbDist_low_bandPass,2)
            DO i1 = LBOUND(OutData%TurbDist_low_bandPass,1), UBOUND(OutData%TurbDist_low_bandPass,1)
              OutData%TurbDist_low_bandPass(i1,i2,i3,i4) = REAL(ReKiBuf(Re_Xferred), SiKi)
              Re_Xferred = Re_Xferred + 1
            END DO
          END DO
        END DO
